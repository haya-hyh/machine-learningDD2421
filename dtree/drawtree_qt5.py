...
def draw(p, t, x, y):
    if isinstance(t, dtree.TreeLeaf):
        p.drawText(int(x-3), int(y+15), 'T' if t.cvalue else 'F')  # 转换为整数
        return x, x+20
    xx = x
    anchors = []
    for b in t.branches:
        mid, xx = draw(p, t.branches[b], xx, y+70)
        p.drawText(int(mid-3), int(y+68), str(b))  # 转换为整数
        anchors.append(mid)
    newMid = (x+xx)/2
    p.drawText(int(newMid-7), int(y+15), t.attribute.name)  # 转换为整数
    p.drawEllipse(int(newMid-15), int(y), 30, 20)  # 转换为整数
    for m in anchors:
        p.drawLine(int(newMid), int(y+20), int(m), int(y+70))  # 转换为整数
    return newMid, xx+10
